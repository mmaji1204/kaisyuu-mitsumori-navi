...
import { RelatedArticles } from "@/components/BlogArticleCard";
...
export default async function BlogArticlePage({ params }: Props) {
  ...
  return <>
    ...
    <article className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-10">
      ...
      <div className="mt-10 space-y-10">{article.sections.map(s => <section key={s.id} id={s.id} className="scroll-mt-6"><h2 className="mb-5 border-l-4 border-orange-500 pl-3 text-2xl font-black leading-relaxed">{s.title}</h2>{s.paragraphs.map(p => <p key={p} className="mt-4 leading-8">{p}</p>)}{s.checklist && <ul className="mt-5 list-disc space-y-3 rounded-xl bg-slate-50 py-5 pl-10 pr-5 leading-8">{s.checklist.map(c => <li key={c}>{c}</li>)}</ul>}{s.links && <ul className="mt-5 space-y-3 text-sm leading-7">{s.links.map(link => <li key={link.href}><Link href={link.href} className="font-bold text-green-800 underline underline-offset-4">{link.title}</Link></li>)}</ul>}</section>)}</div>
