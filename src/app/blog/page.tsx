...
export default function BlogPage() {
  return <>
    ...
    {[...additionalArticles, preparationArticle].map(article => <article key={article.slug} className="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <Link href={`/blog/${article.slug}`} className="group block sm:grid sm:grid-cols-[0.8fr_1.2fr]">
        ...
        <div className="p-6"><time dateTime={article.publishedAt} className="text-sm text-slate-600">2026年9月21日</time><h2 className="mt-3 text-xl font-black leading-relaxed group-hover:text-orange-700">{article.title}</h2><p className="mt-3 leading-7 text-slate-700">{article.description}</p><p className="mt-4 font-bold text-green-800 underline">記事を読む</p></div>
      </Link>
    </article>)}
  </>;
}
